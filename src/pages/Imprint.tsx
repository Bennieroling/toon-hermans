...
const SECTION_KEYS = ["entity", "contact", "responsibility", "disputes"] as const

export function Imprint(): ReactNode {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground antialiased">
      ...
      <main className="mx-auto max-w-3xl px-6 py-16 lg:py-24">
        ...
        <p className="mt-8 text-base leading-7 text-foreground sm:text-lg">{t("imprint.intro")}</p>
        <div className="mt-12 space-y-10">
          {SECTION_KEYS.map((key) => (
            <section key={key}>
              <h2 className="font-display text-xl font-bold tracking-[-0.02em] text-foreground sm:text-2xl">
                {t(`imprint.sections.${key}.heading`)}
              </h2>
              <p className="mt-3 text-base leading-7 text-muted-foreground">
                {t(`imprint.sections.${key}.body`)}
              </p>
            </section>
          ))}
        </div>
